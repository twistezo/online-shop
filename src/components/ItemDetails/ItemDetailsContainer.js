...
class ItemDetailsContainer extends Component {
  handleAddToCart = () => {
    this.props.onAddToCartClick(parseInt(this.props.routeParamId));
  };

  handleAddReview = (name, review) => {
    this.props.onAddReview(
      new Review(name, new Date(), review),
      parseInt(this.props.routeParamId)
    );
  };
  ...
  render() {
    ...
    return (
      <div>
        {DataUtils.isValidNum(idFromUrl) ? (
          <this.Container
            item={DataUtils.getItemById(initialItems, parseInt(idFromUrl))}
